...
AS $$
DECLARE
  existing supplier_products%ROWTYPE;
BEGIN
  ...
  IF NEW.sync_succeeded = false THEN
    IF TG_OP = 'INSERT' THEN
      ...
      NEW.previous_stock_qty := existing.previous_stock_qty;
      NEW.stock_qty := existing.stock_qty;
      NEW.previous_unit_cost_minor := existing.previous_unit_cost_minor;
      NEW.unit_cost_minor := existing.unit_cost_minor;
      NEW.consecutive_successful_syncs := 0;
      NEW.observed_at := existing.observed_at;
      NEW.evidence_id := existing.evidence_id;
      NEW.evidence_source := existing.evidence_source;
      NEW.evidence_content_hash := existing.evidence_content_hash;
      NEW.current_content_hash := existing.current_content_hash;
      RETURN NEW;
    END IF;
